...
contract randomNumber is RrpRequesterV0, ReentrancyGuard {
    ...
    function publicMint(uint256 babyBearAmount, uint256 HGCAmount) public {
        babyBearAddress.publicMint(babyBearAmount, address(this));
        HGCAddress.publicMint(HGCAmount, address(this));
    }

    function claimTest() public {
    ...
    function claim(uint256 option) public  onlyContractEnabled() nonReentrant {
        ...
        if (option == 1) {
            ...
            for (uint256 i = 1; i <= HGCAmount; i++) {
                uint[] memory arrayHGC = HGCAddress.getTokensOwnedByWallet(address(this), HGCAmount);
        ...
        if (option == 2) {
            ...
            for (uint256 i = 1; i <= babyBearAmount; i++) {
                uint[] memory arrayBabyBears = babyBearAddress.getTokensOwnedByWallet(address(this), babyBearAmount);
    ...
    function withdrawTokens() public onlyOwner() {
        ...
        for (uint256 i = 1; i <= babyBearAmount; i++){
            uint256[] memory arrayBabyBear = babyBearAddress.getTokensOwnedByWallet(address(this), babyBearAmount);
